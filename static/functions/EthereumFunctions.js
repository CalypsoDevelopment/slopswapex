...
// const BSCTokens = require('~/static/tokenLists/BSCTokenList.json')
const chains = require('~/static/SlopSwapSmart/SlopSwapSmartContracts')

const FACTORY = require('~/static/artifacts/ISlopSwapFactory.json')
const PAIR = require('~/static/artifacts/ISlopSwapPair.json')
const ERC20 = require('~/static/artifacts/ERC20.json')
...
export function checkNetwork (provider) {
...
export function getFactory (address, signer) {
  return new ethers.Contract(address, FACTORY.abi, signer)
}
...
export async function getDecimals (token) {
  const decimals = await token.decimals().then((result) => {
    ...
    alert('No tokenDecimals function for this token, set to 0')
    return 0
  })
  return decimals
}
...
export async function fetchReserves (address1, address2, pair, signer) {
  try {
    ...
    const coin1 = new ethers.Contract(address1, ERC20.abi, signer)
    const coin2 = new ethers.Contract(address2, ERC20.abi, signer)
